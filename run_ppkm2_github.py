...
# Chapter 4 pPKM2 differentiation assay.
# This script produces the primary pPKM2 analysis bundle used for Figure 5,
# Table 3, and the related Chapter 4 supplementary outputs.
ROOT = Path(__file__).resolve().parent
PARSED_INPUT_PATH = ROOT / "parsed_data" / "ppkm2" / "per_cell_stats_all.csv"
ORIGINAL_CSV_DIR = ROOT / "original_data" / "ppkm2" / "csvs"
RX_0929_PREFIX = re.compile(r"^(?!N4_0929)0929(?=[_ ]|$)", flags=re.IGNORECASE)
...
def combine_csv_dir(csv_dir: Path) -> pd.DataFrame:
    ...
    csv_paths = sorted(path for path in csv_dir.glob("*.csv") if "partial" not in path.name.lower())
    if not csv_paths:
        raise FileNotFoundError(f"No pPKM2 CSVs found in {csv_dir}")
    df = pd.concat((pd.read_csv(path) for path in csv_paths), ignore_index=True, axis=0)
...
def ensure_parsed_input_exists() -> Path:
    # Rebuild the parsed pPKM2 table locally if the repo only contains raw uploaded
    # `original_data/ppkm2/csvs/*.csv` files.
    if PARSED_INPUT_PATH.exists():
        return PARSED_INPUT_PATH

    print("[prep] Parsed pPKM2 input missing; rebuilding it from original_data/ppkm2/csvs")
    ...
    print(f"[prep] Wrote parsed pPKM2 input: {PARSED_INPUT_PATH}")
    return PARSED_INPUT_PATH
...
def main() -> None:
    ...
    print("\nFinished pPKM2 RM-ANOVA pipeline")
