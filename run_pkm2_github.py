...
# Chapter 4 PKM2 differentiation assay.
# This script produces the PKM2 companion analysis bundle referenced alongside
# Figure 5 and the related supplementary Chapter 4 outputs.
ROOT = Path(__file__).resolve().parent
PARSED_INPUT_PATH = ROOT / "parsed_data" / "pkm2" / "per_cell_stats_all.csv"
ORIGINAL_CSV_DIR = ROOT / "original_data" / "pkm2" / "csvs"
RX_0929_PREFIX = re.compile(r"^(?!N4_0929)0929(?=[_ ]|$)", flags=re.IGNORECASE)
...
def combine_csv_dir(csv_dir: Path) -> pd.DataFrame:
    # The Chapter 4 bundles are already per-LIF/per-batch CSV exports, so the
    # rebuild step is simply "collect them all, skip partials, then concatenate".
    csv_paths = sorted(path for path in csv_dir.glob("*.csv") if "partial" not in path.name.lower())
    if not csv_paths:
        raise FileNotFoundError(f"No PKM2 CSVs found in {csv_dir}")
    df = pd.concat((pd.read_csv(path) for path in csv_paths), ignore_index=True, axis=0)
...
def fix_0929_names(df: pd.DataFrame) -> pd.DataFrame:
    # Some Chapter 4 files use two equivalent filename prefixes for the same
    # acquisition day. Normalize them so downstream joins stay consistent.
    out = df.copy()
...
def ensure_parsed_input_exists() -> Path:
    # Rebuild the parsed PKM2 table locally if the repo only contains raw uploaded
    # `original_data/pkm2/csvs/*.csv` files.
    if PARSED_INPUT_PATH.exists():
        return PARSED_INPUT_PATH

    print("[prep] Parsed PKM2 input missing; rebuilding it from original_data/pkm2/csvs")
    df = combine_csv_dir(ORIGINAL_CSV_DIR)
    df = fix_0929_names(df)
    ensure_parent(PARSED_INPUT_PATH)
    df.to_csv(PARSED_INPUT_PATH, index=False)
    print(f"[prep] Wrote parsed PKM2 input: {PARSED_INPUT_PATH}")
    return PARSED_INPUT_PATH
...
def build_config() -> ClusteredAnalysisConfig:
    return ClusteredAnalysisConfig(
        ...
        dataset_label="PKM2",
        analysis_name="chapter_4_pkm2_github_parsed_clustercomp_rm_anova",
        input_mode="chapter4_combined",
        input_candidates=(PARSED_INPUT_PATH,),
        analysis_dir=ROOT / "outputs" / "pkm2_output_V1",
...
def run_pipeline(config: ClusteredAnalysisConfig) -> dict[str, object]:
    # Make the script runnable from the raw repo bundle alone.
...
def main() -> None:
    ...
    print("\nFinished PKM2 RM-ANOVA pipeline")
